...
async function fetchData() {
  ...
  const requestHash = ethers.keccak256(
    ethers.toUtf8Bytes(`kyc_verification_request_agent_${AGENT_ID}`)
  );
...
app.get("/", async (req, res) => {
  try {
    ...
    const capsHtml = d.meta?.capabilities
      ? d.meta.capabilities.map(c => `<div class="cap-tag">${c}</div>`).join("")
      : "";
    const scoreOffset = 220 - (d.score / 100) * 220;
...
.tx-link{display:inline-flex;align-items:center;gap:6px;font-family:'Share Tech Mono',monospace;font-size:11px;color:var(--dim);text-decoration:none;margin-top:10px;transition:color 0.2s;}
...
footer{margin-top:40px;text-align:center;font-family:'Share Tech Mono',monospace;font-size:11px;color:var(--dim);letter-spacing:1px;}
...
<div class="wrap">
  <header>
    <div>
      <div class="eyebrow">ERC-8004 // Arc Testnet</div>
      <h1>Kebs <span>Agent</span></h1>
    </div>
  ...
  <div class="grid">
    <div class="card identity">
      ...
      <div class="card-value big gold">#${d.agentId}</div>
      <div class="card-sub">Owner: ${d.ownerShort}</div>
      <a class="tx-link" href="https://testnet.arcscan.app/token/${IDENTITY_REGISTRY}/instance/${d.agentId}" target="_blank">↗ View on ArcScan</a>
    </div>
    <div class="card reputation">
      ...
      <div class="score-wrap">
        <div class="ring-wrap">
          ...
          <div class="ring-label">${d.score}</div>
        </div>
        <div>
          <div class="card-value green">${d.score}/100</div>
          <div class="card-sub">tag: ${d.tag}</div>
        </div>
      </div>
    </div>
    <div class="card validation">
      ...
      <div class="card-value blue">${d.valResponse === 100 ? 'PASSED' : 'FAILED'}</div>
      <div class="val-badge">✓ ${d.valTag}</div>
      <div class="card-sub" style="margin-top:10px">validator: ${d.valValidator}</div>
    </div>
    <div class="card wallet">
      <div class="card-icon">👛</div>
      <div class="card-label">Owner Wallet</div>
      <div class="card-value mono purple">${d.ownerShort}</div>
      <div class="card-sub" style="margin-top:8px">Gas Balance</div>
      <div class="card-value purple" style="font-size:20px;font-weight:700;margin-top:4px">${d.balance} ARC</div>
    </div>
    <div class="card full">
      <div class="card-icon">📋</div>
      <div class="card-label">Agent Metadata</div>
      <div class="meta-grid">
        <div class="meta-item"><div class="k">Name</div><div class="v">${d.meta?.name || 'Kebs Protocol Agent'}</div></div>
        <div class="meta-item"><div class="k">Type</div><div class="v">${d.meta?.agent_type || 'defi'}</div></div>
        <div class="meta-item"><div class="k">Version</div><div class="v">${d.meta?.version || '1.0.0'}</div></div>
        <div class="meta-item"><div class="k">Platform</div><div class="v">${d.meta?.platform || 'Arc Testnet'}</div></div>
      </div>
      <div style="margin-top:14px;font-size:13px;color:var(--dim);line-height:1.6">${d.meta?.description || ''}</div>
      <div class="caps">${capsHtml}</div>
    </div>
  </div>
  <div class="timestamp">Last fetched: ${d.fetchedAt}</div>
  <footer>
    Built on <a href="https://arc.network" target="_blank">Arc Testnet</a> &nbsp;·&nbsp;
    ERC-8004 Agent Identity &nbsp;·&nbsp;
    <a href="https://github.com/mohamedkibrahim3419-hub" target="_blank">Kebs Protocol</a>
  </footer>
</div>
</body>
</html>`);
